<?php
	...
	if (!isset($_GET['product'])) header("Location: ./catalog.php");

	$product_id = $_GET['product'];

	if (isset($_POST["search_submit"])) {
		$search_input = $_POST["search_input"];
		$product_detail = read_data("SELECT * FROM products WHERE name LIKE '%$search_input%'");
	} else {
		$product_detail = read_data("SELECT * FROM products WHERE product_id = $product_id");
	}

	$category_id = $product_detail[0]["category_id"];
	$category = read_data("SELECT name FROM categories WHERE category_id = $category_id");
...
<head>
  ...
  <meta name="keywords" content="">
	<title><?= $product_detail[0]["name"] ?> - Living Barn Indoensia</title>
...
<body class="text-neutral-800 font-lato bg-neutral-50">
	...
	<main class="px-5 sm:px-10 md:px-12 lg:px-16 xl:px-24 xl:mx-auto xl:max-w-1366px">
		<section class="pt-136px sm:pt-24">
			<section class="pt-5 sm:pt-0">
				<p>
					<span>
						<a href="./catalog.php">Catalog</a>
					</span>  >  
					<span class="font-bold"><?= $product_detail[0]["name"] ?></span>
				</p>
			</section>
			<section class="mt-5 sm:grid sm:grid-cols-7 sm:gap-x-5">
				<section class="h-60 sm:h-64 md:h-72 lg:h-96 bg-neutral-200 rounded-2xl sm:col-span-3">
					<img src="./assets/database/img/<?= $product_detail[0]["picture"] ?>" alt="" class="w-full h-full object-cover object-center rounded-2xl">
				</section>
				<section class="mt-10 sm:mt-6 lg:mt-4 sm:col-span-4">
					<h2 class="font-bold"><?= $category ?></h2>
					<h1 class="text-xl font-bold"><?= $product_detail[0]["name"] ?> - <?= $product_detail[0]["size"] ?></h1>
					<p class="pt-2">
            <span>⭐⭐⭐⭐⭐</span>
            <span class="text-xs"><?= $product_detail[0]["review"] ?> Reviews</span>
          </p>
          <p class="pt-7">Rp. <?= $product_detail[0]["price"] ?></p>
          <p class="pt-3 text-neutral-600 text-justify sm:text-left">
          	<?= $product_detail[0]["short_desc"] ?>
          </p>
          <p class="pt-10">Stock: <?= $product_detail[0]["stock"] ?></p>
          <div class="pt-3">
          	<form action="" method="POST" class="w-fit flex items-center space-x-3">
              <button type="submit" name="add_to_cart" value="<?= $product_detail[0]["product_id"] ?>" <?= in_array($product_detail[0]["product_id"], $items) ? 'disabled' : '' ?> class="px-6 py-2.5 ring-2 ring-green-600 ring-inset rounded-lg">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#16A34A" stroke="#16A34A" stroke-width=".5" class="bi bi-cart" viewBox="0 0 16 16">
                  <path d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .491.592l-1.5 8A.5.5 0 0 1 13 12H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5zM3.102 4l1.313 7h8.17l1.313-7H3.102zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-7 1a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm7 0a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"/>
                </svg>
              </button>
			...
			<section class="mt-11 sm:mt-12 md:mt-14 lg:mt-16">
				<p class="text-lg font-bold">Description</p>
				<p class="mt-4 lg:mt-7 text-neutral-600">
					<?= $product_detail[0]["long_desc"] ?>
				</p>
				<?php
					if ($product_detail[0]["discount"] != ""):
						$discounts = json_decode($product_detail[0]["discount"])
				?>
					<p class="mt-4 lg:mt-7 text-neutral-600">
						Get the best deal by placing a minimum order for this product! Enjoy advantageous discounts such as a 
						<?php foreach ($discounts[1] as $key => $discount): ?>
							<?= $discount ?>% discount for orders ≤ <?= $discounts[0][$key] ?> boxes<?= ($key == count($discounts[0]) - 1) ? "." : "," ?>
						<?php endforeach ?>
						Alternatively, contact our <a href="https://wa.me/+6281339224800?text=Halo,%20Living%20Barn!" target="_blank" class="text-green-600">customer service</a> for other special offers.
					</p>
				<?php endif ?>
				<?php
					if ($product_detail[0]["packaging"] != ""):
						$packaging = json_decode($product_detail[0]["packaging"])
				?>
					<p class="mt-4 lg:mt-7 text-neutral-600">
						<!-- Please note that the quantity of your order is specified in boxes, with each box containing <?= $packaging[1] ?> <?= $packaging[0] ?>s. -->
						Please note that the product is packaged in <?= $packaging[0] ?>, with each box containing <?= $packaging[1] ?> <?= $packaging[0] ?>s.
					</p>
				<?php endif ?>
			</section>
		</section>
