<?php
	...
	if (isset($_POST["reset"])) {
		if (reset_password($_POST) === true) header("Location: ./login.php");
	}
?>
...
<head>
  ...
  <meta name="keywords" content="">
	<title>Reset Password - Living Barn Indonesia</title>
...
<body class="text-neutral-800 font-lato bg-neutral-50">
	...
	<header class="fixed-header fixed w-full h-24 bg-neutral-50">
		<div class="h-full px-5 sm:px-10 md:px-12 lg:px-16 xl:px-24 xl:mx-auto xl:max-w-1366px flex justify-between items-center">
			...
			<!-- NAV. -->
			<nav>
				<a href="./login.php" class="block text-center hover:font-bold" style="width: 35.27px">Back</a>
			</nav>
		</div>
	</header>
			...
			<div class="pt-10 sm:pt-20 lg:pt-2 pb-20 sm:pb-24 md:pb-28 lg:pb-40 sm:mx-auto sm:w-96 md:w-auto md:grid md:grid-cols-4 lg:grid-cols-6">
				...
				<form action="" method="POST" class="col-span-2">
					<section class="space-y-2 5">
						<h1 class="text-green-600 text-2xl font-bold font-mont">Reset Password</h1>
						<p class="text-neutral-600">Please provide the following information to reset your password.</p>
					</section>
					<section class="pt-7 grid grid-cols-2 gap-5">
						...
						<div class="w-full space-y-1">
							<label for="password" class="text-neutral-400">New Password: </label>
						...
						<div class="w-full space-y-1">
							<label for="password-confirm" class="text-neutral-400">Password Confirm: </label>
							<input type="password" name="password_confirm" id="password-confirm" placeholder="••••••" minlength="8" required maxlength="32" class="w-full p-4 text-neutral-800 bg-transparent ring-2 ring-neutral-400 ring-inset hover:ring-green-600 focus:ring-0 focus:outline-1 focus:outline-offset-0 focus:outline-green-600 rounded-xl">
						</div>
					</section>

					<button type="submit" name="reset" class="w-full mt-5 py-2.5 text-neutral-50 font-bold bg-green-600 hover:ring-2 hover:ring-green-600 hover:ring-inset hover:text-green-600 hover:bg-transparent focus:ring-1 focus:outline-1 focus:outline-offset-0 focus:outline-green-600 rounded-xl">Reset</button>
				</form>
				<div class="md:col-span-1 lg:col-span-2"></div>
			</div>
		</div>
